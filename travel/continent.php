        ...
        <section class="portfolio-block projects-cards" style="padding-top:0px; margin-bottom:0px; margin-top:15px;">
            <div class="container">
                <div class="heading" style="padding-bottom:0px;">
                    <h2>Images for: <?php echo $continent_name?></h2>
                </div>
                <div class="row">
                   
<!-- Running php code to fetch image detail from database and load it to site -->
<?php
$query_country = "SELECT * from geocountries WHERE Continent = '$continent';";
$result_country = $conn->query($query_country);
if($result_country->num_rows>0){
    while($row_country=$result_country->fetch_assoc())
    {
    $ISO = $row_country['ISO'];
    $query="SELECT travelimage.Path, travelimage.UID, travelimage.ImageID,travelimagerating.Rating,travelimagedetails.title, travelimagedetails.Description
    from travelimage 
    ...
    WHERE  travelimagedetails.CountryCodeISO = '$ISO'
